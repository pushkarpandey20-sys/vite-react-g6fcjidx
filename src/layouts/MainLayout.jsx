...
export default function MainLayout({ children, sidebar, portalLabel, portalColor }) {
  ...
  return (
    <div className="layout-root">
      {/* Top Nav — two groups: nav-left (hamburger+logo+badge) and nav-r (actions) */}
      <nav className="tnav" style={{ background: '#1a0a00', borderBottom: '1.5px solid rgba(212,160,23,0.2)' }}>

        {/* ── Left group ── */}
        <div className="nav-left">
          {/* Hamburger — CSS controls display (none on desktop, flex on mobile) */}
          ...
          {portalLabel && (
            <div className="nav-portal-badge" style={{ padding: '3px 10px', borderRadius: 20, fontSize: 9, fontWeight: 800, letterSpacing: '.5px', fontFamily: "'Cinzel', serif", background: `${portalColor}22`, color: portalColor, border: `1px solid ${portalColor}55`, whiteSpace: 'nowrap' }}>
              {portalLabel}
            </div>
          )}
        </div>
        ...
        {/* ── Right group ── */}
        <div className="nav-r">
          {/* Test Login — hidden on small screens via CSS class */}
          {isUser && !devoteeId && (
          ...
          <div className="nav-user-pill"
            style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(61,31,0,0.6)', padding: '6px 14px', borderRadius: 20, cursor: 'pointer', border: '1.5px solid rgba(255,107,0,0.25)', boxShadow: '0 2px 8px rgba(0,0,0,0.3)', overflow: 'hidden', flexShrink: 0 }}
            onClick={() => isUser && !devoteeId && setShowLogin(true)}>
            <div style={{ fontSize: 12, fontWeight: 900, color: '#F0C040', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center', gap: 4, maxWidth: 100 }}>
              {devoteeId && isUser
                ? <><IconUser size={14} color="#F0C040" /><span className="nav-username">{devoteeName.split(' ')[0]}</span></>
                : isPandit ? '🪔'
                : isUser ? <><IconUser size={14} color="#F0C040" /><span className="nav-username">Login</span></>
                : '⚙️'}
            </div>
            {devoteeId && isUser && (
              <div className="nav-logout-btn" onClick={e => { e.stopPropagation(); logout(); }} style={{ fontSize: 10, color: '#FF6B00', fontWeight: 800, flexShrink: 0, padding: '2px 6px', background: 'rgba(255,107,0,0.08)', borderRadius: 8, marginLeft: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
