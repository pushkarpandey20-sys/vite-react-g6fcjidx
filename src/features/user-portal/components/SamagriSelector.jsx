  ...
  return (
    <div className={`samagri-kit-card card ${selected ? 'selected' : ''}`} 
      ...
      <div className="kit-includes" style={{ background: '#f9f9f9', padding: '12px', borderRadius: '10px' }}>
        <div style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', color: '#8B6347', marginBottom: '8px', letterSpacing: '0.5px' }}>Items Included</div>
        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px' }}>
          {kit.itemsIncluded?.map((item, idx) => (
            <li key={idx} style={{ fontSize: '12px', color: '#2C1A0E', display: 'flex', alignItems: 'center', gap: '5px' }}>
      ...
      <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ fontSize: '13px', fontWeight: 700 }}>{selected ? 'Samagri Added' : 'Add to Ritual'}</span>
      </div>
    </div>
  );
}
