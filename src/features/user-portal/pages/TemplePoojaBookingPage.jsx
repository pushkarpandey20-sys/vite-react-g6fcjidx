...
export default function TemplePoojaBookingPage() {
  ...
  return (
    <div className="temple-booking-grid">
      <div style={{ padding: '30px', background: 'rgba(26,15,7,0.88)', borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.4)', border: '1px solid rgba(212,160,23,0.18)' }}>
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '30px' }}>
          <div style={{ width: '80px', height: '80px', background: 'rgba(61,31,0,0.6)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px' }}>{temple.icon}</div>
          <div>
        ...
        <form onSubmit={handleBooking}>
          <div style={{ marginBottom: '30px' }}>
            <label style={{ display: 'block', marginBottom: '12px', fontWeight: 800, color: '#F0C040', fontSize: 14 }}>🔮 Select Sacred Pooja</label>
            <div style={{ display: 'grid', gap: '10px' }}>
              {(temple.poojas || []).map(p => (
                <div key={p} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px', borderRadius: '15px', border: `2px solid ${selectedPooja === p ? '#FF6B00' : 'rgba(212,160,23,0.18)'}`, background: selectedPooja === p ? 'rgba(255,107,0,0.06)' : 'rgba(40,15,5,0.7)', cursor: 'pointer', transition: '0.2s' }} onClick={() => setSelectedPooja(p)}>
                  ...
                  <span style={{ fontWeight: 800, color: 'rgba(255,248,240,0.85)' }}>{p}</span>
                  <span style={{ marginLeft: 'auto', fontSize: '13px', color: 'rgba(255,248,240,0.55)', fontWeight: 700 }}>₹{basePrice}/-</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px', marginBottom: '30px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontWeight: 800, color: '#F0C040', fontSize: 13 }}>Pooja Date</label>
              <input type="date" style={{ padding: '10px 14px', borderRadius: 10, border: '1.5px solid rgba(212,160,23,0.3)', fontSize: 14, color: 'rgba(255,248,240,0.85)', background: 'rgba(40,15,5,0.7)', outline: 'none' }} required value={options.date} onChange={e => setOptions({...options, date: e.target.value})} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontWeight: 800, color: '#F0C040', fontSize: 13 }}>Names (For Sankalp)</label>
              <input style={{ padding: '10px 14px', borderRadius: 10, border: '1.5px solid rgba(212,160,23,0.3)', fontSize: 14, color: 'rgba(255,248,240,0.85)', background: 'rgba(40,15,5,0.7)', outline: 'none' }} placeholder="Rahul, Priya, etc." value={options.familyNames} onChange={e => setOptions({...options, familyNames: e.target.value})} />
            </div>
          </div>

          <div style={{ background: 'rgba(26,15,7,0.85)', padding: '20px', borderRadius: '18px', border: '1px dashed rgba(212,160,23,0.3)', marginBottom: '30px' }}>
            <h4 style={{ marginBottom: '15px', color: '#F0C040', fontWeight: 800 }}>✨ Exclusive Devotional Add-ons</h4>
            <div style={{ display: 'grid', gap: '12px' }}>
              {[
                ...
                <label key={key} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', cursor: 'pointer',
                  background: options[key] ? 'rgba(255,107,0,0.05)' : 'rgba(40,15,5,0.7)',
                  border: `1.5px solid ${options[key] ? '#FF6B00' : 'rgba(212,160,23,0.18)'}`,
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', fontWeight: 800, color: 'rgba(255,248,240,0.85)' }}>{label}</div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,248,240,0.55)', marginTop: 2 }}>{sub}</div>
                  </div>
            ...
            {options.prasadDelivery && (
              <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontWeight: 800, color: '#F0C040', fontSize: 13 }}>Delivery Address (Verified Home)</label>
                <textarea required placeholder="Full address with Pincode..." value={options.deliveryAddress} onChange={e => setOptions({...options, deliveryAddress: e.target.value})}
                  style={{ padding: '10px 14px', borderRadius: 10, border: '1.5px solid rgba(212,160,23,0.3)', fontSize: 14, color: 'rgba(255,248,240,0.85)', background: 'rgba(40,15,5,0.7)', outline: 'none', minHeight: 80, resize: 'vertical' }} />
              </div>
            )}
          </div>
      ...
      <aside>
        <div style={{ padding: '20px', position: 'sticky', top: '20px', background: 'rgba(26,15,7,0.88)', borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.4)', border: '1px solid rgba(212,160,23,0.18)' }}>
          <h4 style={{ marginBottom: '15px', color: '#FF6B00' }}>🛕 About this Shrine</h4>
          <div style={{ fontSize: '13px', color: 'rgba(255,248,240,0.6)', lineHeight: 1.6, marginBottom: '20px' }}>
      ...
      {submitting && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,4,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
